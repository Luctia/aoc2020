...
part1 = ((\x -> (lookfor 1 x) * (lookfor 3 x)) . countDifferences [] . sort . map (\x -> read x::Int) . lines) <$> readFile "day10input.txt"
...
part2 = (countAllPosLinks . sort . map (\x -> read x::Int) . lines) <$> readFile "day10input.txt"